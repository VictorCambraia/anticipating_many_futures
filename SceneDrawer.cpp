...
//---------------------------------------------------------------------------
// slightly adjusted to visualize predictions
//---------------------------------------------------------------------------
//---------------------------------------------------------------------------
// Includes
//---------------------------------------------------------------------------
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
* Draw_Prediction() :  draws T steps forward
*  - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
void Draw_Prediction( vector<vector<XnPoint3D> > pose  )
{
    vector<int> pa;
    pa.push_back(-1);pa.push_back(0);pa.push_back(1); pa.push_back(2);pa.push_back(2);pa.push_back(3);pa.push_back(4);pa.push_back(5);pa.push_back(6);
    int drsize = 5;
    for(int t = 0; t < pose.size() ; t++){
        vector<XnPoint3D> temp_pose = pose[t];

        for(int p = 0; p < temp_pose.size(); p++){
            XnPoint3D pt;
            pt = temp_pose[p];
            g_DepthGenerator.ConvertRealWorldToProjective(1, &pt, &pt);
            drawCircle(pt.X, pt.Y, drsize);

            if(pa[p]>=0){
                XnPoint3D pt2[2];
                pt2[0] = temp_pose[p];
                pt2[1] = temp_pose[pa[p]];

                g_DepthGenerator.ConvertRealWorldToProjective(2, pt2, pt2);
                #ifndef USE_GLES
                    glVertex3i(pt2[0].X, pt2[0].Y, 0);
                    glVertex3i(pt2[1].X, pt2[1].Y, 0);
                #else
                    GLfloat verts[4] = {pt[0].X, pt2[0].Y, pt2[1].X, pt2[1].Y};
                    glVertexPointer(2, GL_FLOAT, 0, verts);
                    glDrawArrays(GL_LINES, 0, 2);
                    glFlush();
                #endif
...
void DrawDepthMap(const xn::DepthMetaData& dmd, const xn::SceneMetaData& smd, vector<vector<XnPoint3D> > vec_pred )
{
